...
import { useProducts, useAdminProducts } from "@/lib/products";
...
function AdminProducts() {
  ...
  const { remove, addOrUpdate } = useAdminProducts();
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-border/60 hover:bg-secondary/30">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="font-medium truncate">{r.name.en}</p>
                    ...
                    <td>
                      ...
                      {r.oldPrice && <div className="text-xs text-muted-foreground line-through">{formatPrice(Number(r.oldPrice))}</div>}
                    </td>
                    <td>{r.stock}</td>
                    <td className="p-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button onClick={() => handleRemove(r.id)} className="p-2 rounded-lg hover:bg-destructive/10 text-destructive" aria-label="Delete"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
                {rows.length === 0 && <tr><td colSpan={5} className="p-10 text-center text-muted-foreground">No products yet.</td></tr>}
              </tbody>
            </table>
          </div>
        )}
...
function ProductForm({ initial, onClose, onSaved }: { initial: Product | null; onClose: () => void; onSaved: () => void }) {
  ...
  const { addOrUpdate } = useAdminProducts();
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload: Product = {
      ...
      name: {
        ...
        ar: String(fd.get("name_ar") || "").trim() || "",
      },
      category: String(fd.get("category")),
      price: Number(fd.get("price")),
      oldPrice: fd.get("old_price") ? Number(fd.get("old_price")) : undefined,
      image: imagePreview,
      stock: Number(fd.get("stock") || 0),
      badge: (fd.get("badge") ? String(fd.get("badge")) : undefined) as Product["badge"],
      rating: initial?.rating ?? 4.8,
      reviews: initial?.reviews ?? 0,
    };
    
    const desc = String(fd.get("description_en") || "").trim();
    if (desc) {
        // We use intersection type in hook, we can just cast here
        (payload as any).description = desc;
    }
    
    setBusy(true);
    addOrUpdate.mutate(payload, {
      onSuccess: () => {
        toast.success(initial ? "Updated successfully" : "Created successfully");
        setBusy(false);
        onSaved();
      },
      onError: (err) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-foreground/40 backdrop-blur-sm transition-all">
      <form onSubmit={onSubmit} className="bg-background rounded-3xl border border-border/60 shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-6 grid sm:grid-cols-2 gap-5 overflow-y-auto">
          <Field label="Name (EN)"><input name="name_en" defaultValue={initial?.name.en} required className={inputCls} /></Field>
          <Field label="Name (AR)"><input name="name_ar" defaultValue={initial?.name.ar ?? ""} className={inputCls} /></Field>
          ...
          <Field label="Old Price (Rs.)"><input name="old_price" type="number" step="1" min="0" defaultValue={initial?.oldPrice ?? ""} placeholder="Optional discount" className={inputCls} /></Field>
          ...
          <div className="sm:col-span-2"><Field label="Description (EN)"><textarea name="description_en" defaultValue={(initial as any)?.description ?? ""} rows={3} placeholder="Describe the product details and material..." className={inputCls} /></Field></div>
        </div>
